...
(function ($) {
    ...
    function InitData() {
        ...
        $.getJSON("public/js/address/zip3.js", function (r) {
            var zipDataPack = {};
            //郵遞區號與縣市區域對照表
...
function setAddressInput($container) {
    ...
    $inpZip = $container.find("input.cZip:first");
    ...
    // $(document).on("change",$inpZip, function() {
    //     //非台灣時停用
    //     if ($inpCountry.val() != "TW") return;
    ...
    // $(document).on("autocomplete",$inpZip, function(t) {
    //     //非台灣時停用
    //     if ($inpCountry.val() != "TW") return [];
